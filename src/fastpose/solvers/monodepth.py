...
The kernels are built by `build_monodepth_kernels(jit, ...)` so the same
source compiles for the CPU (`njit`) and for CUDA (`cuda.jit(device=True)`).
Its five callees are passed in rather than imported: on the CPU they must be
the very kernels the other solver modules already instantiated (a second
instance would duplicate their on-disk cache entries), and for CUDA they must
come from the GPU instantiation, since a device function cannot call an
`njit` global that touches arrays.

Each `*_core` takes the six coordinate columns as a 6-tuple rather than the
8-element `data` tuple the CPU driver passes: the two hybrid weights are
neither per-point nor needed by the solvers, and on the GPU they ride in the
problem's `params` vector (see cuda/problem.py).
"""
...
# int64 scratch the two Sturm-based solvers need, shared by the CPU wrappers
# below and by the CUDA kernels, which allocate it as per-thread local arrays
WORKSPACE_INTS = 133


def build_monodepth_kernels(jit, real_roots_sturm, p3p_impl,
                            solve_3xn_neg, project_rotation,
                            charpoly_danilevsky_n):
    ...
    @jit(fastmath=True)
    def _solve_monodepth_shift_3pt_core(data, sample, models, d, coeffs, C0, C1, coef, chain, roots, lo_stack, hi_stack, A, B,
        degs, slo_stack, shi_stack):
        # scratch is passed in pre-shaped: see the note above the factory
        x1_x, x1_y, x2_x, x2_y, dd1, dd2 = data
        ...
        n_roots = real_roots_sturm(coef, 4, chain, degs, roots,
    ...
    @jit(fastmath=True)
    def _solve_monodepth_p3p_core(data, sample, models, xs, Xs, X01, X02, XXinv, C, pq, taus):
        # scratch is passed in pre-shaped: see the note above the factory
        x1_x, x1_y, x2_x, x2_y, dd1, dd2 = data
        ...
        count = p3p_impl(xs, Xs, X01, X02, XXinv, C, pq, taus, models)
        inv_d2 = 1.0 / d2_0
    ...
    @jit(fastmath=True)
    def _solve_monodepth_shared_focal_3pt_core(data, sample, models, a, b, c, dd, C0, C1, AM, coef, chain, roots, lo_stack,
        hi_stack, row, tmp_row, A, B, degs, slo_stack, shi_stack):
        # scratch is passed in pre-shaped: see the note above the factory
        x1_x, x1_y, x2_x, x2_y, dd1, dd2 = data
        ...
        if not charpoly_danilevsky_n(AM, coef, row, tmp_row, 4):
            return 0
        n_roots = real_roots_sturm(coef, 4, chain, degs, roots,
                                    lo_stack, hi_stack, slo_stack, shi_stack)
    ...
    @jit(fastmath=True)
    def _solve_monodepth_varying_focal_3pt_core(data, sample, models, a, b, A3, B3, A, B):
        # scratch is passed in pre-shaped: see the note above the factory
        x1_x, x1_y, x2_x, x2_y, dd1, dd2 = data
    ...
    return {
        'pose_from_point_triplets': _pose_from_point_triplets,
        'refine_suv': _refine_suv,
        'solve_monodepth_shift_3pt_core': _solve_monodepth_shift_3pt_core,
        'solve_monodepth_p3p_core': _solve_monodepth_p3p_core,
        'solve_monodepth_shared_focal_3pt_core': _solve_monodepth_shared_focal_3pt_core,
        'solve_monodepth_varying_focal_3pt_core': _solve_monodepth_varying_focal_3pt_core,
    }


_CPU = build_monodepth_kernels(cpu_jit, _real_roots_sturm, _p3p_impl,
                               _solve_3xn_neg, _project_rotation,
                               _charpoly_danilevsky_n)

# module-level names the tests import; these are the CPU kernels, byte-for-byte
# the same source as before the factory
_pose_from_point_triplets = _CPU['pose_from_point_triplets']
_refine_suv = _CPU['refine_suv']
_solve_monodepth_shift_3pt_core = _CPU['solve_monodepth_shift_3pt_core']
_solve_monodepth_p3p_core = _CPU['solve_monodepth_p3p_core']
_solve_monodepth_shared_focal_3pt_core = _CPU[
    'solve_monodepth_shared_focal_3pt_core']
_solve_monodepth_varying_focal_3pt_core = _CPU[
    'solve_monodepth_varying_focal_3pt_core']
...
def _solve_monodepth_shift_3pt(data, sample, models, workspace):
    ...
    # scratch the core wants. CPU-only - `.reshape` does not compile for CUDA,
    ...
    slo_stack = iw[5:69]
    shi_stack = iw[69:133]
    return _solve_monodepth_shift_3pt_core(
        (data[0], data[1], data[2], data[3], data[4], data[5]), sample, models,
        d, coeffs, C0, C1, coef, chain, roots, lo_stack, hi_stack, A, B,
        degs, slo_stack, shi_stack)
...
def _solve_monodepth_p3p(data, sample, models, workspace):
    ...
    return _solve_monodepth_p3p_core(
        (data[0], data[1], data[2], data[3], data[4], data[5]), sample, models,
        xs, Xs, X01, X02, XXinv, C, pq, taus)
...
def _solve_monodepth_shared_focal_3pt(data, sample, models, workspace):
    ...
    slo_stack = iw[5:69]
    shi_stack = iw[69:133]
    return _solve_monodepth_shared_focal_3pt_core(
        (data[0], data[1], data[2], data[3], data[4], data[5]), sample, models,
        a, b, c, dd, C0, C1, AM, coef, chain, roots, lo_stack,
        hi_stack, row, tmp_row, A, B, degs, slo_stack, shi_stack)
...
def _solve_monodepth_varying_focal_3pt(data, sample, models, workspace):
    ...
    return _solve_monodepth_varying_focal_3pt_core(
        (data[0], data[1], data[2], data[3], data[4], data[5]), sample, models,
        a, b, A3, B3, A, B)
