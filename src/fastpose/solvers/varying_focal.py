...
The kernels are built by `build_varying_focal_kernels(jit, ...)` so the same
source compiles for the CPU (`njit`) and for CUDA (`cuda.jit(device=True)`).
Its four callees are passed in rather than imported: on the CPU they must be
the very kernels `solvers/fundamental.py` and `solvers/essential.py` already
instantiated (a second instance would duplicate their on-disk cache entries),
and for CUDA they must come from the GPU instantiation, since a device
function cannot call an `njit` global.

`_solve_varying_focal_core` takes the two principal points as scalars and the
...
def build_varying_focal_kernels(jit, nullspace_7pt, det3_flat,
                                solve_cubic_real, pose_from_essential):
    @jit(inline=True)
    ...
    @jit(fastmath=True)
    def _solve_varying_focal_core(data, sample, models, pp1x, pp1y, pp2x, pp2y,
                                  A, f_a, f_b, tmp, roots, focals_sq, e, Rbuf):
        ...
        if not nullspace_7pt(A, f_a, f_b):
            return 0

        value_1 = det3_flat(f_a)
        value_0 = det3_flat(f_b)
        for j in range(9):
            tmp[j] = 2.0 * f_b[j] - f_a[j]
        value_neg_1 = det3_flat(tmp)
        for j in range(9):
            tmp[j] = 2.0 * f_a[j] - f_b[j]
        value_2 = det3_flat(tmp)
        ...
        n_roots = solve_cubic_real(cubic, quadratic, linear, value_0, roots)
        ...
        for r in range(n_roots):
            ...
            num_poses = pose_from_essential(e, (x1_x, x1_y, x2_x, x2_y), sample,
    ...
    return {
        'rybkin_f_sq': _rybkin_f_sq,
        'rybkin_focals_sq': rybkin_focals_sq,
        'solve_varying_focal_core': _solve_varying_focal_core,
    }


_CPU = build_varying_focal_kernels(cpu_jit, _nullspace_7pt, _det3_flat,
                                   _solve_cubic_real, _pose_from_essential)

# module-level names the tests import; these are the CPU kernels, byte-for-byte
# the same source as before the factory
_rybkin_f_sq = _CPU['rybkin_f_sq']
rybkin_focals_sq = _CPU['rybkin_focals_sq']
_solve_varying_focal_core = _CPU['solve_varying_focal_core']
...
def _solve_varying_focal_7pt(data, sample, models, workspace):
    ...
    return _solve_varying_focal_core((x1_x, x1_y, x2_x, x2_y), sample, models,
                                     pp1x, pp1y, pp2x, pp2y, A, f_a, f_b, tmp,
                                     roots, focals_sq, e, Rbuf)
