...
@njit(cache=True, inline='always', fastmath=True)
def _sampson_dsdF(x, y, xp, yp, f, max_error_sq, dsdF):
    # truncated Sampson residual of one correspondence for the flat 3x3
    # matrix f, plus its gradient w.r.t. the matrix entries (row-major).
    # Returns (residual, valid); valid is False outside the threshold.
    ...
    if residual * residual >= max_error_sq * denominator:
        return 0.0, False
...
def monodepth_hybrid_cost(model, data, max_error_sq, best_score):
    # truncated hybrid cost for calibrated monodepth models
    ...
    for i in range(n):
        ...
        if weight_sampson > 0.0:
            ...
            if r2 < max_error_sq:
                cost += weight_sampson * r2
                num_inliers += 1
            else:
                cost += weight_sampson * max_error_sq
        ...
        if scale_reproj > 0.0:
            ...
            if Z1z > 0.0:
                ...
                cost += r2 if r2 < max_error_sq else max_error_sq
...
@njit(cache=True, fastmath=True)
def monodepth_focal_hybrid_cost(model, data, max_error_sq, best_score):
    # truncated hybrid cost for monodepth focal models [R | t | f1 | f2 |
    # scale] in centered pixel coordinates
    x1_x, x1_y, x2_x, x2_y = data[0], data[1], data[2], data[3]
    d1 = data[4]
    d2 = data[5]
    scale_reproj = data[6]
...
def _make_accumulate_calibrated(num_tangent):
        ...
        for i in range(n):
            ...
            if scale_reproj > 0.0:
                ...
                if Z1z > 0.0:
                    ...
                    rx = px - xp
                    ry = py - yp
                    if scale_reproj * (rx * rx + ry * ry) < max_error_sq:
                        num_residuals += 2
                        ...
                        if refine_shift:
                            ...
                            J1[8] = 0.0
                        for p in range(num_tangent):
                            Jtr[p] += scale_reproj * (J0[p] * rx + J1[p] * ry)
                            for q in range(p, num_tangent):
                                JtJ[p, q] += scale_reproj * (J0[p] * J0[q]
                                                             + J1[p] * J1[q])
            ...
            if weight_sampson > 0.0:
                s_i, valid = _sampson_dsdF(x, y, xp, yp, e, max_error_sq, dsdF)
                if valid:
                    num_residuals += 1
                    for p in range(6):
                        acc = 0.0
                    ...
                    for p in range(6, num_tangent):
                        J[p] = 0.0
                    for p in range(num_tangent):
                        Jtr[p] += weight_sampson * J[p] * s_i
                        for q in range(p, num_tangent):
                            JtJ[p, q] += weight_sampson * J[p] * J[q]
...
def _make_accumulate_focal(shared):
    ...
    def _accumulate(data, f, state, JtJ, Jtr, max_error_sq):
        ...
        for i in range(n):
            ...
            if weight_sampson > 0.0:
                s_i, valid = _sampson_dsdF(x, y, xp, yp, fmat, max_error_sq,
                                           dsdF)
                if valid:
                    num_residuals += 1
                    for p in range(num_tangent):
...
_accumulate_calibrated = _make_accumulate_calibrated(7)
_accumulate_calibrated_shift = _make_accumulate_calibrated(9)
_accumulate_shared_focal = _make_accumulate_focal(True)
_accumulate_varying_focal = _make_accumulate_focal(False)
...
class LMMonoDepthPoseRefiner():
    # calibrated monodepth: pose + scale (7 tangent parameters)
